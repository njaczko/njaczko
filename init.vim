...
" TODO coc.nvim is commented out to expriment with using built-in completion
" for simple editing. Sometime, lets enable this for certain filetypes
" (probably just Go)
...
imap jj <Esc>
imap jk <Esc>
" TODO experimenting with replacing this with something more advanced...
" imap kk <Down><CR>
...
" note: a big drawback compated to coc is that this isn't async. it can take a
" long time to open the completion menu when a large buffer is open
...
set pumheight=10
" disable the auto pairs plugin from creating an imapping for <space>, which (interferes with above)
